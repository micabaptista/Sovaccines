...
bool acceptValues(char * inputFile){
    ...
    if(atoi(fgets(line, sizeof(line), fp)) > 0 && atoi(fgets(line, sizeof(line), fp)) > 0   && atoi(fgets(line, sizeof(line), fp))
        && atoi(fgets(line, sizeof(line), fp)) > 0 && atoi(fgets(line, sizeof(line), fp)) && !strcmp(fgets(line, sizeof(line), fp), "") 
        && !strcmp(fgets(line, sizeof(line), fp),"") && atoi(fgets(line, sizeof(line), fp)) > 0){ 
        valid=true;
    }
    fclose(fp);
    return valid;
